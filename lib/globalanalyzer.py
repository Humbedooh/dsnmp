...
def analyze_server(config, server, group, http_url):
    global gissues, goutput, gtoutput, jobs_done, no_jobs, served_ourselves
    ...
    goutput.append( "<tr><td><a href='%s/%s/%s.html'>%s</a></td><td>%s</td><td>%s</td></tr>" % (
        http_url,
        group,
        server,
        server,
...
def start_analysis(group, config, dry_run, settings):
    ...
    threads = []
    for server in sorted(config['hosts']):
        thread = Thread(target = analyze_server, args = [config, server, group, http_url])
        thread.start()
    ...
    if not served_ourselves:
        with open("%s/%s/index.html" % (http_dir, group), "w") as f:
            ...
            f.write(templates.html_output_template % ("<h2>Overall status for %s:</h2>" % group, "\n".join(sorted(goutput))))
            f.close()
            
    snmp_pages[group]['index'] = templates.html_output_template % ("<h2>Overall status for %s:</h2>" % group, "\n".join(sorted(goutput)))
